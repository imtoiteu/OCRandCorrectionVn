...
from services import ocr_service, text_service
from services import markdown_normalize   # repair unmatched $$ before persisting OCR markdown
from services import smart_ocr_service    # OCR dispatcher (pure extraction; no AI)
...
def _persist_and_index(fid, kind, text, meta=None, do_index=True):
    """Persist derived text for a document (A2).

    Best-effort: a failure here never breaks the originating request. The OCR viewer
    and Documents library reuse these persisted artifacts. (`do_index` is retained for
    call-site compatibility but is now a no-op — RAG/chat indexing was removed.)
    """
...
def _run_page_ocr(path: Path, page: int, apply_ai: bool, image_path_for_ocr: str, engine_name: str):
    # Pure OCR extraction only. The optional LLM post-correction path ("AI Cleanup")
    # was removed; `apply_ai` is accepted for signature compatibility and ignored.
