...
class ProtonxProvider(CorrectionProvider):
    ...
    def __init__(self, model_id: str = DEFAULT_MODEL_ID, device: str | None = None,
        ...
        try:
            ...
            # AutoTokenizer dispatches to T5TokenizerFast which crashes
            # on BPE vocab stored as a dict (no spiece.model present).
            self._AutoTokenizer = PreTrainedTokenizerFast
    ...
    def _resolve_device(self) -> str:
        t = self._torch
        mps_ok = bool(getattr(getattr(t, "backends", None), "mps", None)) and t.backends.mps.is_available()
        d = self._device
        if d in (None, "auto"):
