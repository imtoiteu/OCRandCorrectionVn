B-5 First solution: Put $Q= x_{1}^{2}+\cdots+x_{n}^{2}.$ Since Q is homogeneous, P is divisible by Q if and only if each of the homogeneous, P is divisible by Q if and only if each of the homogeneous components of P is divisible by Q. It is thus sufficient to solve the problem in case P itself is homogeneous, say of degree d.
...
Third solution: (by Richard Stanley) Write $x= (x_{1},\dots,x_{n})$ and $\nabla=\left(\frac{\partial}{\partial x_{1}},\dots,\frac{\partial}{\partial x_{n}}\right).$ Suppose that $P(x)=Q(x)\left(x_{1}^{2}+\dots+x_{n}^{2}\right).$ Then
